...
/// without a real I/O fault. `finalize` is scripted independently via `finalize_fails`.
...
impl DurableLineWriter for ScriptedWriter {
    ...
    fn finalize(&mut self) -> std::result::Result<(), FinalSyncFailures> {
        if self.finalize_fails {
            // Report both the file and directory sync as failed, so a test can assert both typed
            // slots are retained independently.
