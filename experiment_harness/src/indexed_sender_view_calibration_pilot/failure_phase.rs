...
pub(crate) enum FailurePhase {
    /// The environment or plumbing failed, independently of what was being measured. The only phase
    /// a retry can be earned from, and only before the first measured sample.
    Infrastructure,
    ...
    /// measurement.
    ///
    /// Its own phase because it is neither: `/proc` becoming unreadable is not the system under test
    /// producing a result, but neither is it a prospective interruption a retry may silently
    /// replace. The spec makes both host-observation failures non-retryable, and giving them a
    /// distinct phase records that as a fact about the failure rather than mislabelling a harness
    /// fault as an application one to reach the same eligibility.
    HarnessObservation,
}
