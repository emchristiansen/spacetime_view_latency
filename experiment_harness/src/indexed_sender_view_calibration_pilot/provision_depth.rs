...
/// distribution or server. `PartialProvision`'s own variants carry facts that can only be observed
/// from real provisioning capabilities, so a rule expressed only over those variants could not be
/// proven anywhere but a live run.
...
impl ProvisionDepth {
    ...
    /// reaps its own child and data directory before returning `Err`.
