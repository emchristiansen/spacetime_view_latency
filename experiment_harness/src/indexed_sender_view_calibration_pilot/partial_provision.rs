...
/// one before it and never dropping one, so a record states how far acquisition got without a reader
/// having to reconstruct it from a diagnostic string.
///
...
/// the spec requires failed attempts to retain provision/runtime/module facts.
///
...
pub(crate) enum PartialProvision {
    ...
    /// The module WASM was read, hash-verified against the committed constant, and staged; no server
    /// started.
    ModuleStaged {
