//! Why a candidate hex string is not a canonical git release commit.

use std::fmt;

use crate::manifest::canonical_hex_shape_error::CanonicalHexShapeError;

/// The exhaustive typed reason a hex string is not a canonical git release commit. A canonical release
/// commit is exactly [`ReleaseCommit::CANONICAL_HEX_LEN`](super::release_commit::ReleaseCommit::CANONICAL_HEX_LEN)
...
///
/// This is the release-commit-specific *translation* of the shared
/// [`CanonicalHexShapeError`] (`From<CanonicalHexShapeError>`): the shape algorithm is single-sourced,
/// but this remains a distinct domain type so a release-commit failure can never be paired with the
...
pub(crate) enum ReleaseCommitParseError {
    /// The string is not the canonical release-commit-hex length: `expected` is
    /// [`ReleaseCommit::CANONICAL_HEX_LEN`](super::release_commit::ReleaseCommit::CANONICAL_HEX_LEN),
...
impl From<CanonicalHexShapeError> for ReleaseCommitParseError {
    /// Translate the shared shape failure into the release-commit-specific typed error, one variant to one.
...
impl fmt::Display for ReleaseCommitParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Length { expected, observed } => write!(
                f,
                "release commit hex must be exactly {expected} lowercase hex characters, found {observed}"
            ),
            Self::NonHex {
                offending_index,
                offending_byte,
                ...
                f,
                "release commit hex has a non-hexadecimal byte {offending_byte:#04x} at index {offending_index}"
            ),
            Self::NonCanonicalCase {
                offending_index,
                offending_byte,
                ...
                f,
                "release commit hex has an uppercase (non-canonical) byte {offending_byte:#04x} at \
