...
/// raises from `error make`, and two is the conventional argument-misuse status. A code the waiter
/// might also produce by accident would make a refusal indistinguishable from the failure that
/// happened to collide with it, which is the whole defect this constant exists to close.
///
/// **Defined here and passed to the waiter as an argument**, rather than written into the script
/// and re-declared here. The decoder is the only party that has to know the number, so it is the
/// only party that names it; a copy in the script would be free to drift from this one, silently,
/// and drift in exactly this value silently reclassifies evidence.
...
///
/// The distinction that matters is not pass/fail but **whether the host was measured at all**. A
...
///
/// **Stopping is not a prediction that the fault is permanent.** Several of those failures are
/// plainly transient and the next invocation might well succeed. It is that this invocation can no
/// longer demonstrate that what it measures was gated, and continuing past a gate whose integrity is
/// gone would produce records that claim a guarantee the run cannot support.
///
/// Collapsing those two into "nonzero" is what let an unmeasured host be recorded as a refused one.
