...
/// sort locale, and manifest-stream order, and the Step 1 review rejected an earlier digest for
/// carrying none of them — so a record that could name the digest without the recipe would preserve
/// exactly the defect that review removed.
...
impl GeneratedTreeDigest {
    /// The digest accepted at `ControlRegistry` Step 1.
    ///
    /// Validated rather than trusted: the constant is a hand-transcribed hex string, so its length
    /// and alphabet are checked here and a malformed digest fails loudly at startup instead of
    /// being written into every record of a completed screen.
