...
//! text, so a failed prerequisite can never precede a measured write on a missing message_visibility row.
...
fn a_prerequisite_failure_is_rejected() {
    ...
    assert!(
        rendered.contains("message_visibility write index 5"),
