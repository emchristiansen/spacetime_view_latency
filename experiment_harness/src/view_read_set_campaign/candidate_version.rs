...
///
/// Numbered independently of the completed seed-7 Pilot's version of the same name. That campaign
/// measured a cumulative walk on one server; this one measures a fixed-cardinality scale point on a
/// fresh server, so the two are different measured paths and their version lines do not continue
/// each other.
