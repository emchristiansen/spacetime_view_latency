...
//!
//! Duplicated per test tree rather than shared across them, as the observation sink's fixture of the
//! same name is, so each tree owns its own label namespace.
...
pub(super) fn scratch_dir(label: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("observed-row-set-{label}-{}", std::process::id()));
    if dir.exists() {
