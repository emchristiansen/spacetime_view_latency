...
/// after-E2 expectation with the final after-E1 row set, or two expectations from different roles or
/// scale points. [`CompositionTransitionExpectation`] is minted as a whole from one attempt's
...
/// owner; the owned count is exact and unchanged across phases, since the schedule updates in place;
/// the foreign count matches
...
/// seeded everywhere before, and the schedule's derived final payload on each owned key after, with
/// the foreign slice seeded throughout. It then requires the witness row to appear in both
/// observations with a changed payload, since the pinned SpacetimeDB source elides a byte-identical
/// update outright.
///
/// **What it does not cover.** E2's per-sample visibility, which lives in the paced channel's own
/// samples. This is the composition claim spanning the attempt, not a record of every intermediate
/// state.
///
...
/// trusting that a validator once returned `Ok`. The embedded transition, the per-range counts, and
/// the witness row's key and both payloads are recorded so the check can be reproduced without
/// reading this module.
///
...
/// rows satisfy this specific recorded transition.
...
impl ValidatedComposition {
    /// Check both observed result sets against their own side of `expected`, minting the artifact
    /// only if every applicable gate holds.
    ///
    /// The witness is named by an [`OwnedSliceOffset`] rather than a raw key: the witness must be a
    /// row the schedule actually targets, and an arbitrary `u64` could name a foreign row — which no
    /// measured write ever touches, so its payload is unchanged by construction and it would witness
    /// visibility that never happened. The offset can only have come from a walk of the frozen owned
    /// slice, and the owned key is derived from it.
    ///
    ...
    pub(crate) fn validate(
        ...
        todo!(
            "Phase 2: census each retained row set against its own side of the transition, \
             rejecting any row outside both key ranges and any row whose owner is not its range's \
             expected identity; require the owned count to be exact and identical across the two \
             phases, since the schedule updates in place; require the foreign count to match \
             ExpectedForeignVisibility (zero for the Arm — the security gate) and every foreign row \
             to carry the seeded payload in both phases; match each side's ExpectedPayloadState, \
             requiring the seeded payload on every owned row for Seeded and the schedule's \
             final_payload_at_offset for each owned slice offset after the measured batch; and \
             require the witness key, derived as OWNED_KEY_BASE plus the witness offset, to be \
             present in both observations with a changed payload, since a byte-identical update is \
             elided at the pinned commit"
        )
    }
    ...
    /// The phase-matched transition this finding was checked against, embedded so the artifact is
    /// self-contained.
