...
//! proves — an execution-order prefix, a scale-matched composition finding, and above all that the
//! whole is genuinely *short* of complete — is carried by the constructor rather than by the field
//! types. A private field is visible to its declaring module **and every descendant**, so a
//! `#[cfg(test)] mod tests` child, or any child added later, could write the struct literal for four
//! channels plus a validated composition and thereby record a complete attempt as a failure.
//! `sealed` has no children, so [`PartialEvidence::sealed`] really is the only door.
...
mod sealed {
    ...
    /// Whatever an attempt had produced when it failed: an execution-order prefix of the channels,
    /// and the composition finding if that check had already run.
    ///
    /// Deliberately a different type from
    /// [`ScalePointEvidence`](crate::view_read_set_campaign::scale_point_evidence::ScalePointEvidence)
    /// rather than the same type with optional fields: only a complete attempt can seal that one, so
    /// "analysis selects one *complete* attempt per logical slot" is enforced by the types rather
    /// than by a check someone must remember.
    ///
    /// **The one state this excludes.** All four channels *and* a validated composition — because
    /// that combination is not a partial attempt at all, it is exactly the evidence
    ...
    ///   failed during composition validation, or before its observed rows could be durably
    ///   retained.
    ///
    /// That last case is why the bound is `<= CHANNEL_COUNT` rather than `<`. The spec freezes the
    /// order of the four channels but says nothing about where composition validation sits among
    /// them, so a driver that happens to validate composition first would make it unreachable — and a
    /// driver that does not, would not. Relying on that would be assuming an ordering contract that
    /// has not been frozen.
    ///
    /// An empty value is likewise a real state: the attempt reached a measurable configuration and
    /// died before the first channel finished.
    ///
    /// **What a retained composition means.** [`ValidatedComposition`] is unforgeable, so `Some` is a
    /// finding that really was produced against retained rows, kept rather than discarded merely
    /// because the attempt later failed. `None` says only that the check had not completed.
    ...
    impl PartialEvidence {
        ...
        /// [`TerminalAttemptRecord::sealed`](crate::view_read_set_campaign::terminal_attempt_record::TerminalAttemptRecord)
        /// to prove the retained evidence belongs to the identity it is filed under.
...
pub(crate) use sealed::PartialEvidence;
