...
//!
//! The inventory and its order derivation live in the private, *childless* inline module [`sealed`]
//! because everything the inventory claims — the exact attempt count, and that no logical slot is
//! repeated — is carried by [`AttemptInventory::sealed`] rather than by the field's type. A private
//! field is visible to its declaring module **and every descendant**, so a `#[cfg(test)] mod tests`
//! child, or any child added later, could write `AttemptInventory { order }` around a permutation
//! that measures one rung twice and another never — which is exactly the design error the
//! duplicate-slot check exists to catch. `sealed` has no children, so the two constructors below
//! really are the only source of values.
//!
//! [`CAMPAIGN_ATTEMPT_COUNT`] stays outside `sealed`: it is a derived `usize` constant, and a
//! constant carries no invariant that sole minting could protect.

use crate::view_read_set_campaign::campaign_params::{
    PILOT_BLOCKS_USIZE, UNRELATED_GLOBAL_ROWS_LADDER_LEN,
};
...
mod sealed {
    ...
    use crate::view_read_set_campaign::campaign_params::{CAMPAIGN_SEED, RUNG_ORDER_DOMAIN};
    ...
    /// *this* list rather than generating attempts as it goes, an attempt that is never reached is
    /// still a known member and can be recorded
    /// [`NotRun`](crate::view_read_set_campaign::attempt_outcome::AttemptOutcome::NotRun) — which is
    /// what makes "report generation fails on missing or duplicate planned identities" checkable.
    ///
    ...
    /// inventory metadata: analysis reconstructs ladders by scale-point identity, never by ledger
    /// order, and the adjacency of a rung's two roles is not an inferential pairing.
    ...
    impl AttemptInventory {
        ...
        ///
        /// Takes no seed: unlike the completed Pilot, whose block order came from a CLI argument,
        /// this campaign's order is derived from the frozen [`CAMPAIGN_SEED`], so there is no
        /// run-time input that could produce a different preregistration.
        ///
        /// The order is the spec's, in three layers:
        ///
        /// 1. **Blocks ascend.** No permutation — the randomization that matters is within the
        ///    block.
        ...
        ///    rung therefore occupies each execution position within one of at most `⌈blocks/r⌉`
        ///    counts of each other — exact balance when the block count divides by the ladder length,
        ///    and the closest possible balance otherwise, without moving the frozen block count.
        /// 3. **The two roles of a rung are adjacent**, with the first alternating by block parity —
        ///    Arm first on even blocks, Control first on odd. Each role independently walks the
        ///    block's prescribed rung sequence, matched roles never drift apart across the campaign,
        ///    and first-role counts are exactly balanced for an even block count and differ by one
        ///    for the five-block Pilot.
        ...
        /// Seal a built order, failing loud unless it is the right count with no logical slot
        /// repeated.
        ///
        /// Duplicate-slot rejection is the half that matters: a permutation bug emitting one rung
        /// twice and another never would still produce sixty attempts, so a bare length check would
        /// pass it and the campaign would silently measure the wrong design.
        fn sealed(order: Vec<AttemptKey>) -> Result<Self> {
            ...
            for (position, attempt) in order.iter().enumerate() {
                for (other_position, other) in order.iter().enumerate().skip(position + 1) {
                        ...
                        "the frozen campaign inventory repeats a logical slot at positions \
    ...
    ///
    /// Copies [`Schedule::seeded_permutation`](crate::plan::schedule::Schedule): each ladder position
    /// is assigned a [`RungOrderKey`] and the vector is sorted by it. Sorting a `Vec` is a
    /// permutation, so the result is exactly `0..len` reordered — no position added, dropped, or
    /// duplicated.
    ...
    /// `key=value` coordinates following the domain, digested as UTF-8. It is built from stable
    /// numeric values — never `Debug` output — so the derived order is reproducible against the
    /// recorded seed and cannot be moved by a Rust rename.
    ...
    ///
    /// A rotation rather than a fresh permutation per block is what makes the design *near-balanced*
    /// by construction: across blocks, each rung visits execution positions in a cycle, so every
    /// position count differs by at most one. A per-block reshuffle would only be balanced on
    /// average.
    fn cyclic_rung_order(base: &[usize], block: PilotBlockIndex) -> Vec<usize> {
        ...
        (0..len)
            .map(|offset| base[(offset + shift) % len])
            .collect()
    }
    ...
    ///
    /// The pair is always exactly `{Arm, Control}`; only the order varies, alternating by block
    /// parity. Parity rather than a seeded bit because the requirement is *balance* across blocks,
    /// and a derived bit would only balance in expectation.
...
pub(crate) use sealed::AttemptInventory;
