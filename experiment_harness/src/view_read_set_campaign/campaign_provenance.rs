...
/// recorded separately by [`AttemptProvenance`](super::attempt_provenance::AttemptProvenance).
///
...
///
/// [`HarnessBuildProvenance`] is reused from the completed Pilot's module rather than duplicated. It
/// projects [`BuildProvenance`], a fact about the *harness binary* rather than about either
/// campaign's experiment ontology, and a second projection could drift from it. The coupling is
/// real and worth stating plainly: a change to that enum changes both ledgers' shape, so it must be
/// treated as a shared contract rather than as `entity_owner_pilot`'s private type.
