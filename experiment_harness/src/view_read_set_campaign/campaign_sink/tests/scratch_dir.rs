...
//!
//! Duplicated per test tree rather than shared across them, as this campaign's other filesystem
//! fixtures of the same name are, so each tree owns its own label namespace.
...
pub(super) fn scratch_dir(label: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("campaign-sink-{label}-{}", std::process::id()));
    if dir.exists() {
