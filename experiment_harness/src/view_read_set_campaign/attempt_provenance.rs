...
/// the fresh database instance it published, and the confirmed-read setting its measurement runs
/// under.
///
/// Copies [`AttemptProvenance`](crate::entity_owner_pilot::attempt_provenance::AttemptProvenance),
/// the completed Pilot's same-named record, rather than being shared with it: that type is the shape
/// of evidence already on disk under a different campaign's vocabulary, and adding a field to it
/// would change how those lines deserialize. The assembly is identical — value-copying immutable
/// scalars out of the still-live capabilities, exactly as
/// [`ValidatedRunManifest`](crate::manifest::validated_run_manifest::ValidatedRunManifest) does. It
/// holds no handle and no borrow, so it makes no claim the process is still alive; it says only
/// which instance produced the evidence recorded under the same attempt identity.
///
/// **`confirmed_reads` is recorded, and its limits are these.** The spec requires every new
/// provenance record to carry it explicitly so a ledger-only check can verify it without inferring
/// it from build provenance. It is read from [`CONFIRMED_READS`], which is the same constant
/// [`ConnectedClient::connect`](crate::client::connected_client::ConnectedClient::connect) passes to
/// the SDK's `with_confirmed_reads` — so this is *what the harness applies*, not an independent
/// read-back of what the connection negotiated, which the SDK builder exposes no way to obtain. It
/// therefore cannot disagree with
/// [`CampaignParameters`](super::campaign_parameters::CampaignParameters)'s copy, and a check that
/// compared the two would be checking a constant against itself.
///
...
impl AttemptProvenance {
    ...
    ///
    /// **Which clauses are real observations, and which is not.** The CLI and standalone versions,
    /// the CLI release commit, and the module digest were read off the live distribution and the
    /// published artifact, so those four can genuinely disagree with the pins and are the substance
    /// of the gate. `confirmed_reads` cannot disagree *within one process*, since both records read
    /// [`CONFIRMED_READS`]; what it catches is a ledger assembled from lines written by two
    /// different builds, which is exactly the case a ledger-only reader has no other way to detect.
    ///
    ...
    pub(crate) fn agrees_with(&self, campaign: &CampaignProvenance) -> Result<()> {
        let _ = campaign;
        todo!(
            "Phase 2: require this attempt's observed CLI version and standalone version to equal \
             CampaignProvenance::expected_version, its CLI release commit to equal \
