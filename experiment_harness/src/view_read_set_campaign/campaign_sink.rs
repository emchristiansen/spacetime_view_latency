...
//! order that never happened, or one that silently extends a torn tail. Establishing that boundary
//! now is the point even though [`CampaignSink::create`] is currently a `todo!()` and no value is
//! reachable at all: Phase 2 makes it reachable, and the topology must already be right when it does.
...
mod sealed {
    ...
    /// implementation, the same [`OutputPath`] and [`RecordSeq`], and the same three-field shape. It
    /// is a separate sink for the same reason the record vocabulary is separate: the write methods
    /// differ only in which record type they accept, and sharing one would mean one ledger schema
    /// for two campaigns.
    ///
    ...
    /// [`CampaignLedgerLine::at`](crate::view_read_set_campaign::campaign_ledger_line::CampaignLedgerLine::at)
    /// is callable elsewhere does not weaken this: a hand-built line is not a written line, and this
    /// sink is the only thing that writes. Every field being private to this childless module is
    /// what makes that hold — the counter cannot be set from outside, only advanced by a successful
    /// write.
    ///
    ...
    impl CampaignSink {
        ...
        pub(crate) fn create(output: &OutputPath) -> Result<Self> {
            ...
            todo!(
                "Phase 2: open the ledger through FileLineWriter::create, which is O_EXCL, so an \
                 ...
                 production still reaches a sink solely through this constructor and the one \
        ...
        /// borrowing write vocabulary — so there is nothing to clone on the way to the line.
        ///
        ...
        pub(crate) fn append(&mut self, record: CampaignRecord) -> Result<RecordSeq> {
            ...
            todo!(
                "Phase 2: refuse immediately if already poisoned, re-reporting the original reason; \
                 otherwise take self.next_seq, build the line with CampaignLedgerLine::at, \
                 serialize it with serde_json and a trailing newline, and write it through the \
                 DurableLineWriter. Advance next_seq only after write, flush, and sync_data have all \
                 returned success. Any serialization or persist failure poisons the sink with a \
                 diagnostic naming the sequence and CampaignRecord::variant_name — the variant name \
                 rather than the rendered body, because on a serialization failure the body cannot \
                 be rendered"
            )
        }
