//! The descriptor's *sole* per-block storage is the 30 [`KeyedBlockOutcome`]s: the authoritative fits it
...
fn sole_outcome_storage_derives_fits_and_convergence() {
    ...
    for (i, fit) in derived_fits.iter().enumerate() {
        assert_eq!(
            fit.identified_beta(),
            outcomes[i].outcome().fit().identified_beta(),
    ...
    // fit and the convergence from one indexed keyed outcome is the migrated indexed-access path.
    for (i, keyed) in outcomes.iter().enumerate() {
        assert!(
            matches!(keyed.outcome().convergence().selection(), BasinSelection::Selected { .. }),
