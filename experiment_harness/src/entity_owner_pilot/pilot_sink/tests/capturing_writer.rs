...
/// A writer that performs no I/O but records every line handed to [`Self::write_line`], so a test
/// can inspect exactly the bytes the sink serialized. It shares its capture buffer with the test via
/// an `Rc<RefCell<…>>` handle returned from [`Self::new`], because the sink takes ownership of the
/// boxed writer. The same fixture the historical
/// [`ObservationSink`](crate::observation::observation_sink::ObservationSink) tests use, over the
/// one shared durability seam.
...
impl CapturingWriter {
    ...
    /// lines after the sink has written.
