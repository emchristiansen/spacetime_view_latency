...
///
/// The latencies are a full sealed batch of distinct samples: sealing an artifact reads only each
/// element's ladder position, so the sample values are irrelevant here — but they must be a *valid*
/// [`RawLatencies`], since a fixture that could not exist in a real run would prove nothing about
/// one. `positions` indexes [`GlobalRowRung::ALL`], the sole source of rung values, so a test cannot
/// name a rung outside the frozen ladder.
