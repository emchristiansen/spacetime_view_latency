...
/// ([`RunAwaitingDose`]), or the ladder is drained and the run advances to disconnect
/// ([`RunDisconnecting`]). Exhaustion of the fixed ladder iterator — not a count — is what moves the run
/// off the dose ladder.
pub(crate) enum RunDoseStep {
    /// A dose remains; the run awaits that dose's observation.
    Awaiting(RunAwaitingDose),
    /// The dose ladder is drained; the run advances to its disconnect transition.
    Exhausted(RunDisconnecting),
}
